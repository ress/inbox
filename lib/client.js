...
var Stream = require("stream").Stream,
    ...
    mailboxNames = require("./names.json"),
...
IMAPClient.prototype._init = function(){
     ...
     * Root mailbox
     */
    this._rootMailbox = new this.Mailbox({client: this});
...
IMAPClient.prototype._handlerTaggedCondstore = function(status){
    if(this._capabilities.indexOf("ID")>=0){
        this._send("ID (" +
                "\"name\" "+this._escapeString(X_CLIENT_NAME) +
                (packageData.version?" \"version\" "+this._escapeString(packageData.version):"") +
                (X_CLIENT_URL?" \"support-url\" "+this._escapeString(X_CLIENT_URL):"") +
                ")", this._handlerTaggedId.bind(this));
    }else{
        this._postReady();
    }
}
...
IMAPClient.prototype._handlerUntaggedLsub = function(list){
    ...
    if(tags.indexOf("\\Noselect")>=0 && path != "INBOX"){
...
IMAPClient.prototype._postAuth = function(){
    ...
    }else if(this._capabilities.indexOf("ID")>=0){
        this._send("ID (" +
                "\"name\" "+this._escapeString(X_CLIENT_NAME) +
                (packageData.version?" \"version\" "+this._escapeString(packageData.version):"") +
                (X_CLIENT_URL?" \"support-url\" "+this._escapeString(X_CLIENT_URL):"") +
...
IMAPClient.prototype._convertFromUTF7 = function(str){
    ...
    try{
        return str.replace(/&([^\-]+)\-/g, function(a, r){
            r = (r || "").replace(/,/g, "/");
            return iconv.convert(new Buffer(r, "base64")).toString("utf-8");
        });
    }catch(E){
        return str;
    }
...
IMAPClient.prototype.Mailbox = function(options){
    options = options || {};
    ...
    this.type = options.type || this.detectType();
    this.delimiter = options.delimiter || this.client._mailboxDelimiter || "";
}

IMAPClient.prototype.Mailbox.prototype.detectType = function(){
...
IMAPClient.prototype.Mailbox.prototype.listChildren = function(callback){
    var path = this.client._escapeString(this.path ? this.path + this.delimiter + "%":"%");

    if(this.client._capabilities.indexOf("SPECIAL-USE")>=0 || this.client._capabilities.indexOf("XLIST")>=0){
        this.gatherChildInfo(path, callback);
        return;
    }
    
    this.listSubscribed(path, callback);
}
...
IMAPClient.prototype.Mailbox.prototype.listSubscribed = function(path, xinfo, callback){
...
IMAPClient.prototype.Mailbox.prototype.createChild = function(name, callback){
    ...
    this.client._send("CREATE "+this.client._escapeString(path), (function(status){
        if(typeof callback == "function"){
            if(status == "OK"){
                callback(null, new this.Mailbox({
                    client: this,
                    path: path,
                    name: name,
 ...
 */
IMAPClient.prototype.listRoot = function(callback){
...
IMAPClient.prototype.openMailbox = function(mailboxName, options, callback){
    ...
    this._send(command + " " + this._escapeString(mailboxName)+(
